...
const doubleTheLetter = function (string) {
  let double = "";
  for (const element of string) {
    double += element.repeat(2);
  }
  return double;
};

const doubleLettersOf = function (strings) {
  return strings.map(doubleTheLetter);
};
...
const charCodesOf = function (strings) {
  return strings.map((letter) => letter.charCodeAt(0));
};
...
const splittedWord = function (string) {
  const newArray = [];
  let word = "";
...
const splitWordsOf = function (strings) {
  return strings.map(splittedWord);
};
...
const rangesOf = function (numbers) {
  return numbers.map(range, numbers);
};
...
const fullNames = function (objects) {
  return objects.map(({ firstName, lastName }) => firstName + " " + lastName);
};
